...
> **v2 — atualizado após teste com GitHub Copilot (feedback endpoint).**
> Melhorias: (1) regra explícita de try/catch obrigatório vs opcional; (2) proibição de lógica de criação de dados no response-builder; (3) exemplo de datetime atualizado para `{ offset: true }`.
...
export const FeedbackOutputSchema = z.object({
  ...
  recordedAt: z.string().datetime({ offset: true }), // aceita ISO com timezone offset
});
...
**Anti-padrão 6 — try/catch ausente em chamadas de serviço:**
```typescript
// DON'T: chamar serviço sem try/catch
const result = await feedbackService.record(parsed.data);
return buildFeedbackResponse(result);

// DO: try/catch obrigatório em toda chamada a src/services/
try {
  const result = await feedbackService.record(parsed.data);
  log.info({ queryId: parsed.data.queryId }, 'Feedback recorded');
  return buildFeedbackResponse(result);
...
// EXCEÇÃO: handlers que não chamam src/services/ estão isentos.
// Documentar com comentário obrigatório:
// No service calls — try/catch not required
```

**Anti-padrão 7 — lógica de criação de dados no response-builder:**
```typescript
// DON'T: response-builder gerando IDs e timestamps
export function createFeedbackOutput(_: FeedbackInput): FeedbackOutput {
  return {
    feedbackId: randomUUID(), // ❌ lógica de negócio no response-builder
    status: 'recorded',
    recordedAt: new Date().toISOString(), // ❌ idem
  };
}

// DO: response-builder só monta HttpResponseInit a partir de dados prontos
// A geração de IDs e timestamps pertence ao serviço em src/services/feedback.ts
export function buildFeedbackResponse(data: FeedbackOutput): HttpResponseInit {
  const parsed = FeedbackOutputSchema.safeParse(data);
  if (!parsed.success) {
    return { status: 500, jsonBody: { error: 'Response validation failed' } };
  }
  return { status: 201, jsonBody: parsed.data };
}
```

**Anti-padrão 8 — `authLevel: 'anonymous'` em endpoints de negócio:**
```typescript
...
*v2 atualizada após teste com feedback endpoint — 3 melhorias incorporadas**  
